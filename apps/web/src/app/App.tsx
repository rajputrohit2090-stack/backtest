...
const strongPassword = z.string().min(12, 'Password must be at least 12 characters').regex(/[A-Z]/, 'Password needs one uppercase letter').regex(/[a-z]/, 'Password needs one lowercase letter').regex(/[0-9]/, 'Password needs one number');
const registerSchema = loginSchema.extend({ name: z.string().min(1), password: strongPassword });
...
function AppShell({ children, fullBleed = false, hideInsights = false }: { children: React.ReactNode; fullBleed?: boolean; hideInsights?: boolean }) {
